...
def train(args):
    ...
    checkpoint = ModelCheckpoint(ckpt, monitor='val_acc', verbose=1, save_best_only=True)
    ...
    history = model.fit_generator(datagen.flow(train_x, train_y), steps_per_epoch=steps, epochs=args.epochs, verbose=1,
                                  callbacks=callback_list, validation_data=datagen.flow(val_X, val_y),
